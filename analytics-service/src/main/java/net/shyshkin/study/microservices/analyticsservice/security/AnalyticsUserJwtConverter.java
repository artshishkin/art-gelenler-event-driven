...
import net.shyshkin.study.microservices.analyticsservice.Constants;
...
public class AnalyticsUserJwtConverter implements Converter<Jwt, AbstractAuthenticationToken> {
    ...
    public AbstractAuthenticationToken convert(Jwt jwt) {
        return Optional.of(jwt)
                ...
                .filter(userDetails -> userDetails instanceof AnalyticsUser)
                .map(userDetails -> (AnalyticsUser) userDetails)
                .map(analyticsUser -> {
                    Collection<GrantedAuthority> authorities = getAuthorities(jwt);
                    analyticsUser.setAuthorities(authorities);
                    return new UsernamePasswordAuthenticationToken(analyticsUser, Constants.NA, authorities);
                })
